...
bool g_horizontalCycleActive{};
POINT g_horizontalCycleAnchor{};
DWORD g_horizontalCycleLastTick{};
int g_horizontalWheelRemainder{};

int g_horizontalCycleNotches{2};
...
std::filesystem::path LayoutFilePath() {
    ...
    ).replace_filename(L"layouts.ini");
}
...
std::filesystem::path StateFilePath() {
    ...
    if (length > 0 && length < path.size()) {
        ...
        directory =
            LayoutFilePath().parent_path();
    }

    std::error_code error;
    std::filesystem::create_directories(directory, error);

    return directory / L"state.ini";
}
...
void LoadMonitorLayouts() {
    ...
    // Migrate old values from layouts.ini if state.ini
    ...
    for (const auto& [monitor, layout] : saved) {
        if (monitor.empty() || layout.empty()) {
            continue;
        }

        const std::wstring normalizedMonitor =
            Lowercase(monitor);

        g_monitorLayouts[normalizedMonitor] =
            layout;

        WritePrivateProfileStringW(
            L"MonitorLayouts",
            normalizedMonitor.c_str(),
            layout.c_str(),
            statePath.c_str()
        );
    }
...
void LoadLayouts() {
    ...
    g_excludedApps.clear();

    for (const auto& [name, enabled] :
         ReadIniSection(L"ExcludedApps", path)) {
    ...
    for (
        const wchar_t* section = sectionNames.data();
        *section;
        section += wcslen(section) + 1
    ) {
        if (
            _wcsicmp(section, L"Settings") == 0 ||
    ...
    if (g_startWithWindows) {
        const std::wstring command =
            ...
            std::filesystem::absolute(
                LayoutFilePath().replace_filename(
                    L"ZoneSmith.exe"
                )
            ).wstring() +
            L"\"";
...
bool IsWindowExcluded(HWND window) {
    ...
    return
        !executable.empty() &&
        g_excludedApps.contains(executable);
}
...
unsigned int ZonesAt(POINT point) {
    ...
    if (!PtInRect(&selectionArea, point)) {
        return 0;
    }
...
RECT ZonesRect(unsigned int zones) {
    ...
    if (
        ...
        const LONG maxHorizontalPadding =
            std::max(
                0L,
                (
                    result.right -
...
bool IsZoneInPreviewSpan(int zone) {
    ...
    for (
        ...
        if (
            (endpoints & (1u << index)) == 0
        ) {
            continue;
        }

        const auto& endpoint =
            CurrentLayout().zones[index];
...
void SelectMonitor(POINT point) {
    ...
    std::wstring newMonitorKey =
        info.szDevice;

    if (
        newMonitorKey.starts_with(
            L"\\\\.\\"
        )
    ) {
        newMonitorKey.erase(
    ...
    const auto saved =
        g_monitorLayouts.find(
            g_currentMonitorKey
        );

    if (
        saved ==
        g_monitorLayouts.end()
    ) {
        return;
    }
...
void ShowOverlay(POINT point) {
    ...
    SetWindowPos(
        g_overlay,
        HWND_TOPMOST,
        g_overlayBounds.left,
        g_overlayBounds.top,
        g_overlayBounds.right -
            g_overlayBounds.left,
        g_overlayBounds.bottom -
            g_overlayBounds.top,
        SWP_NOACTIVATE |
            SWP_SHOWWINDOW
    );
...
void AddTrayIcon() {
    ...
    wcscpy_s(
        g_trayIcon.szTip,
        L"ZoneSmith - Ctrl+Alt+Q to quit"
    );
...
void ApplyPendingOperation() {
    ...
    if (
        ...
        SetWindowPos(
            ...
            current.top,
            g_pendingRestore.size.cx,
            g_pendingRestore.size.cy,
            SWP_NOACTIVATE |
                SWP_NOZORDER
        );

        g_savedWindows.erase(
            g_pendingRestore.window
        );
    }
...
void ResetWindowCycle() {
    ...
    g_horizontalCycleActive = false;
    g_horizontalCycleAnchor = {};
    g_horizontalCycleLastTick = 0;
    g_horizontalWheelRemainder = 0;
}

bool BeginWindowCycle(HWND source) {
    if (!IsCycleWindow(source)) {
        return false;
    }
...
bool HandleTitleBarHorizontalWheel(
    ...
    if (
        g_horizontalCycleActive &&
        now -
            g_horizontalCycleLastTick >
            kHorizontalCycleTimeoutMs
    ) {
        ResetWindowCycle();
    }

    if (!g_horizontalCycleActive) {
        HWND source =
            WindowFromPoint(
                event->pt
            );
...
LRESULT CALLBACK MouseHook(
    ...
    if (code == HC_ACTION) {
        ...
        if (
            ...
            const int dx =
                event->pt.x -
                g_horizontalCycleAnchor.x;

            const int dy =
                event->pt.y -
                g_horizontalCycleAnchor.y;

            if (
                std::abs(dx) >
                    kHorizontalCycleMoveTolerance ||
                std::abs(dy) >
                    kHorizontalCycleMoveTolerance
            ) {
                ResetWindowCycle();
        ...
        //
        // IMPORTANT:
        // This intentionally checks GetAsyncKeyState instead of
        // trusting only cached keyboard-hook state.
        if (
            message ==
                WM_MOUSEWHEEL &&
            !g_leftDown &&
        ...
        // Horizontal wheel over title bar.
        if (
            message ==
                WM_MOUSEHWHEEL &&
            !g_leftDown &&
...
LRESULT CALLBACK KeyboardHook(
    ...
    if (code == HC_ACTION) {
        ...
        if (
            ...
            // Resynchronize against Windows itself so a missed
            // hook event can't leave our internal modifier state stale.
            SyncWindowsKeyState();
...
std::wstring StartupMessage() {
    ...
    return
        ...
        L"\nHorizontal-scroll a title bar to cycle overlapping windows without Win."
        L"\nHorizontal cycle sensitivity: " +
        std::to_wstring(
            g_horizontalCycleNotches
        ) +
        L" wheel notch(es) per window."
...
int WINAPI wWinMain(
    ...
    // Start from the OS's real modifier state rather than assuming
    // the keyboard hook has already seen a Win key transition.
    SyncWindowsKeyState();
